...
$consulta = " SELECT * FROM audit_standard LIMIT 0";
$termino = "";
if (isset($_POST['productos'])) {
    $termino = mysqli_real_escape_string($con, $_POST['productos']);
    $consulta = "SELECT * FROM audit_standard WHERE deleted_at IS NULL AND Title LIKE '%" . $termino . "%'";
}
...
if ($consultaBD && strlen($termino) > 0) {
    ...
    while ($result_data = mysqli_fetch_assoc($consultaBD)) {
        ...
        echo '<td>
            <a href="/admin_audit-standard-edit.php?id=' . $result_data['Id_audit_standard'] . '"
                class="me-2"><i class="bi bi-pencil"></i></a>';

        echo '<a
                href="/includes/basicsettings_audit-standard_delete.php?id=' . $result_data['Id_audit_standard'] . '"><i
