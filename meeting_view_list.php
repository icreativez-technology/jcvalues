...
$_SESSION['Page_Title'] = "Meeting List";
...
$columns = array('id', 'title', 'meeting_id', 'date', 'time', 'status');
...
$currentURI = $_SERVER['REQUEST_URI'];
  ...
  $page = 1;
    ...
    $filterColumns = ['audit_type', 'title', 'audit_area', 'uniq_id', 'audit_std', 'audit_id', 'dept', 'create_dt'];
  ...
  if ($auditType || $uniqID || $auditStd || $auditArea || $deptId || $auditId || $createDt || $title) {
...
<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-disabled">
            ...
            <div class="wrapper d-flex flex-column flex-row-fluid" id="kt_wrapper">
                ...
                <div class="row breadcrumbs">
                    <div class="col-lg-6 col-sm-12">
                        <p><a href="/">Home</a> » <a href="/meeting.php">Meetings</a> »
                            ...
                            <div class="row mt-2 mb-4 ms-2">
                                <div class="col-3">
                                    ...
                                    <!--     <span class="svg-icon svg-icon-1 position-absolute ms-3 mt-2">
                                        ...
                                        <input type="text" data-kt-filemanager-table-filter="search"
                                            ...
                                            placeholder="Search Meeting" id="termino" name="termino" />
                                ...
                                <div class="col-9">
                                    <div class="d-flex justify-content-end">
                                        ...
                                        <button id="btnExport" onclick="fnExcelReport('Meeting.xlsx');"
                            ...
                            <div class="card-body pt-0 table-responsive">
                                <table class="table align-middle table-row-dashed fs-6 gy-5 gx-5"
                                    ...
                                    <thead>
                                        <tr class="text-start text-gray-400 text-uppercase gs-0" data-height="30">
                                            <th class="min-w-100px">
                                               <!--  <a
                                                    href="/meeting_view_list.php?a=meeting&column=title&order=<?php echo $asc_or_desc; ?>&sorting_status=true">
                                                    Title<i
                                                        class="fas fa-sort<?php echo $column == 'title' ? '-' . $up_or_down : ''; ?>"></i>
                                                </a> -->
                                                Title
                                                 <i class="fas fa-sm fa-filter btn" data-toggle="popover_title" id="title"></i>
                                            </th>
                                            <th class="min-w-150px">
                                                <!-- <a
                                                    href="/meeting_view_list.php?a=meetingk&column=meeting_id&order=<?php echo $asc_or_desc; ?>&sorting_status=true">
                                                    Unique ID<i
                                                        class="fas fa-sort<?php echo $column == 'meeting_id' ? '-' . $up_or_down : ''; ?>"></i>
                                                </a> -->
                                                Unique ID
                                            ...
                                            <th class="min-w-100px">
                                                <!-- <a
                                                    href="/meeting_view_list.php?a=meeting&column=date&order=<?php echo $asc_or_desc; ?>&sorting_status=true">
                                                    Date<i
                                            ...
                                            <th class="min-w-100px">
                                             <!--    <a
                                                    href="/meeting_view_list.php?a=meeting&column=time&order=<?php echo $asc_or_desc; ?>&sorting_status=true">
                                                    Time<i
                                                        class="fas fa-sort<?php echo $column == 'time' ? '-' . $up_or_down : ''; ?>"></i>
                                                </a> -->
                                                Time
                                            </th>
                                            <th class="min-w-100px">
                                              <!--   <a
                                                    href="/meeting_view_list.php?a=meeting&column=status&order=<?php echo $asc_or_desc; ?>&sorting_status=true">
                                                    Status<i
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        ...
                                        $sql_data = null;
                                        if ($role == 1) {
                                            $sql_data = "SELECT * FROM meeting WHERE is_deleted = 0 AND meeting.meeting_id LIKE '%$uniqID%' AND meeting.date LIKE '%$createDt%' AND meeting.title LIKE '%$title%' order by " . $columnsQueryItem[$column] . " " . $sort_order;
                                        } else {
                                            $sql_data = "SELECT meeting.* FROM meeting 
                                            LEFT JOIN meeting_participant ON meeting_participant.meeting_id = meeting.id 
                                            WHERE meeting.is_deleted = 0 AND meeting.meeting_id LIKE '%$uniqID%' AND meeting.date LIKE '%$createDt%' AND meeting.title LIKE '%$title%' AND (coordinator = '$empId' OR meeting_participant.participant_id = '$empId') GROUP BY meeting_participant.meeting_id order by " . $columnsQueryItem[$column] . " " . $sort_order;
                                        }
                                        ...
                                        while ($result_data = mysqli_fetch_assoc($connect_data)) {
                                            ...
                                            if ((date("Y-m-d", strtotime($result_data['date'])) < date("Y-m-d", strtotime("today"))) && $result_data['status'] == "Scheduled") {
                                                $result_data['status'] == "Overdue";
                                                $updateQuery = "UPDATE meeting SET status = 'Overdue' WHERE id = '$result_data[id]'";
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?php echo $result_data['meeting_id']; ?>
                                            </td>
                                            <td>
                                                <?php echo date("d-m-y", strtotime($result_data['date'])); ?>
                                            </td>
                                            <td>
                                                <?php echo $result_data['start_time'] . ' ~ ' . $result_data['end_time']; ?>
                                            </td>
                                            <?php
                                            ...
                                            <td>
                                                <a href="meeting_view.php?id=<?php echo $result_data['id']; ?>"
                                                    class="me-3"><i class="fa fa-eye" aria-hidden="true"></i></a>
                                                <?php if ($result_data['status'] != "Cancelled") { ?>
                                                <?php if ($result_data['status'] != "Published") { ?>
                                                <a href="meeting_edit.php?id=<?php echo $result_data['id']; ?>"
                                                    class="me-3 set-url"><i class="bi bi-pencil"
                                                        aria-hidden="true"></i></a>
                                                <?php if ($result_data['status'] != "In Review" && ($role == 1 || $empId == $result_data['coordinator'])) { ?>
                                                <a href="/includes/meeting_status.php?id=<?php echo $result_data['id']; ?>&status=cancel"
                                                    class="me-3"><i class="fa fa-remove" aria-hidden="true"></i></a>
                                                <?php } ?>
                                                <?php } ?>
                                                <?php } ?>
                                                <?php if ($result_data['status'] == "Published" || $result_data['status'] == "In Review") { ?>
                                                <a data-id='<?php echo $result_data['id']; ?>'
                                                    data-unique='<?php echo $result_data['meeting_id']; ?>'
                                                    target="_blank" class="me-3 print-pdf cursor-pointer"><i
                                                        class="bi bi-file-earmark-pdf" aria-hidden="true"></i></a>
                                                <?php } ?>
                                                <?php if ($role == 1) { ?>
                                                <a href="/includes/meeting_delete.php?id=<?php echo $result_data['id']; ?>"
    ...
    <script src="https://cdnjs.cloudflare.com/ajax/libs/html2pdf.js/0.10.1/html2pdf.bundle.min.js"></script>
     ...
     <script>
        ...
        function resetFilter() {
            window.location.href = base_url + '/meeting_view_list.php?a=7';
        }
    ...
    function createPagination(totalPages, page) {
        ...
        let lastPage = ">>";
        liTag +=
            `<li class="page-item m-1"><a href="/meeting_view_list.php?a=meeting&page=${1}&limit=${pageLimit}&column=<?php echo $column; ?>&order=${asc_or_desc}" class="page-link">${firstPage}</a></li>`;
        if (page > 1) {
            liTag +=
                `<li class="page-item m-1"><a href="/meeting_view_list.php?a=meeting&page=${page - 1}&limit=${pageLimit}&column=<?php echo $column; ?>&order=${asc_or_desc}" class="page-link">${prevLabel}</a></li>`;
        }
        if (page == totalPages) {
        ...
        for (var plength = beforePage; plength <= afterPage; plength++) {
            ...
            liTag +=
                `<li class="page-item m-1 ${active}"><a href="/meeting_view_list.php?a=meeting&page=${plength}&limit=${pageLimit}&column=<?php echo $column; ?>&order=${asc_or_desc}" class="page-link">${plength}</a></li>`;
        }
        if (page < totalPages) {
            liTag +=
                `<li class="page-item m-1"><a href="/meeting_view_list.php?a=meeting&page=${page + 1}&limit=${pageLimit}&column=<?php echo $column; ?>&order=${asc_or_desc}" class="page-link">${nextLabel}</a></li>`;
        }
        liTag +=
            `<li class="page-item m-1"><a href="/meeting_view_list.php?a=meeting&page=${totalPages}&limit=${pageLimit}&column=<?php echo $column; ?>&order=${asc_or_desc}" class="page-link">${lastPage}</a></li>`;
    ...
    $('.print-pdf').on('click', function() {
        ...
        $.get(`/includes/meeting_pdf.php?id=${id}`, function(data) {
            let opt = {
