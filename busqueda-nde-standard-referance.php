...
$consulta = " SELECT * FROM nde_standard_referances LIMIT 0";
$termino = "";
if (isset($_POST['productos'])) {
	$termino = mysqli_real_escape_string($con, $_POST['productos']);
	$consulta = "SELECT * FROM nde_standard_referances WHERE title LIKE '%" . $termino . "%'";
}
...
if ($consultaBD && strlen($termino) > 0) {
	...
	while ($result_data = mysqli_fetch_assoc($consultaBD)) {
		...
		if ($result_data['status'] == '1') {
			echo '<td><a href="/includes/nde-standard-referance-status.php?id=' . $result_data['id'] . '"><div class="status-active">Active</div></a></td>';
		} else {
			echo '<td><a href="/includes/nde-standard-referance-status.php?id=' . $result_data['id'] . '"><div class="status-danger">Suspended</div></a></td>';
		}
		echo '<td class="text-end"><a href="/nde-standard-referance-edit.php?id=' . $result_data['id'] . '"><i class="bi bi-pencil"></i></a>';
		echo "
						</tr>
				</tbody>
			";
