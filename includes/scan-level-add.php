...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	$title = $_POST["title"];
	$isExists = "SELECT id FROM scan_levels WHERE title = '$title'";
	...
	if ($result->num_rows == 0) {
		if ($_SESSION['usuario']) {
			...
			$sqlAdd = "INSERT INTO scan_levels (title, created_by) VALUES ('$title', '$userId')";
		...
		header("Location: ../scan-level.php?exist");
	}
} else {
