...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	$title = $_POST["title"];
	$isExists = "SELECT id FROM nde_operators WHERE title = '$title'";
	...
	if ($result->num_rows == 0) {
		if ($_SESSION['usuario']) {
			...
			$sqlAdd = "INSERT INTO nde_operators (title, created_by) VALUES ('$title', '$userId')";
		...
		header("Location: ../nde-operator.php?exist");
	}
} else {
