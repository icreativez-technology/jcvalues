...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	$title = $_POST["title"];
	$isExists = "SELECT id FROM send_for WHERE title = '$title' AND is_deleted = 0";
	...
	if ($result->num_rows == 0) {
		if ($_SESSION['usuario']) {
			...
			$sqlAdd = "INSERT INTO send_for (title, created_by) VALUES ('$title', '$userId')";
		...
		header("Location: ../send-for.php?exist");
	}
} else {
