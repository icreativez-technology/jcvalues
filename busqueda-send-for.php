...
$consulta = " SELECT * FROM send_for WHERE is_deleted = 0 LIMIT 0";
$termino = "";
if (isset($_POST['productos'])) {
	$termino = mysqli_real_escape_string($con, $_POST['productos']);
	$consulta = "SELECT * FROM send_for WHERE is_deleted = 0 AND title LIKE '%" . $termino . "%'";
}
...
if ($consultaBD && strlen($termino) > 0) {
	...
	while ($result_data = mysqli_fetch_assoc($consultaBD)) {
		...
		if ($result_data['status'] == '1') {
			echo '<td><a href="/includes/send-for-status.php?id=' . $result_data['id'] . '"><div class="status-actvie">Active</div></a></td>';
		} else {
			echo '<td><a href="/includes/send-for-status.php?id=' . $result_data['id'] . '"><div class="status-danger">Suspended</div></a></td>';
		}
		echo '<td class="text-end"><a href="/send-for-edit.php?id=' . $result_data['id'] . '"><i class="bi bi-pencil"></i></a>
		<a href="/includes/send-for-delete.php?id=' . $result_data['id'] . '"><i class="bi bi-trash"></i></a></td>';
		echo "
						</tr>
				</tbody>
			";
