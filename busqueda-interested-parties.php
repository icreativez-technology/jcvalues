...
$consulta = "SELECT interested_parties.*, created.First_Name, created.Last_Name, approved.First_Name, approved.Last_Name FROM interested_parties 
LEFT JOIN Basic_Employee as created ON interested_parties.created_by = created.Id_employee
LEFT JOIN Basic_Employee as approved ON interested_parties.approved_by = approved.Id_employee 
...
if ($consultaBD && strlen($termino) > 0) {
    ...
    while ($result_data = mysqli_fetch_assoc($consultaBD)) {
        ...
        echo "</td>";
        echo '<td><a href="/interested-parties-view.php?id='.$result_data['id'].'"><i class="fa fa-eye" aria-hidden="true"></i></a> &nbsp;';
        ?>
