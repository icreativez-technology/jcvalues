...
$_SESSION['Page_Title'] = "NCR MR Approval";
...
<body id="kt_body" class="header-fixed header-tablet-and-mobile-fixed aside-fixed aside-secondary-disabled" onload="loadCharts()">
				...
				<div class="content d-flex flex-column flex-column-fluid" id="kt_content" style="padding: 0;">
					...
					<div class="container-custom" id="kt_content_container">
						<div class="card card-flush">
							...
							<form class="form" action="includes/ncr_mr_approval_update.php" method="post" enctype="multipart/form-data">
									...
									<div id="custom-section-2">
										...
										<table class='table align-middle table-row-dashed fs-6 gy-5'>
											<thead>
												...
												<th class='min-w-50px'>MR Approval</th>
												<th class='min-w-25px text-end'>Actions</th>
											</thead>


											<tbody class='fw-bold text-gray-600' id="mr_approval">
												<?php
												$sql_datos_ncr_mr_approval = "SELECT * From NCR_MR_Approval WHERE Id_ncr = '$_REQUEST[pg_id]'";
												$conect_datos_ncr_mr_approval = mysqli_query($con, $sql_datos_ncr_mr_approval);
												$flag_mr_approval = 0;
												while ($result_datos_mr_approval = mysqli_fetch_assoc($conect_datos_ncr_mr_approval)) {
													$flag_mr_approval++;
												?>
													<tr>

														<?php
														//echo $result_datos_mr_approval['Significant_cause']; 
														?>

														<?php
														$sql_datos_ncr_why_why = "SELECT * From NCR_Why_Why WHERE Id_ncr_why_why = '$result_datos_mr_approval[Id_ncr_why_why]'";
														...
														<td><?php echo $result_datos_mr_approval['Corrective_action']; ?></td>

														<?php
														$sql_data_user = "SELECT * FROM Basic_Employee WHERE Id_employee = '$result_datos_mr_approval[Who_emp]'";
														...
														<td><?php echo $result_datos_mr_approval['When_date']; ?></td>
														<td><?php echo $result_datos_mr_approval['MR_Approval']; ?></td>

														<td class="text-end">
															<a href="/ncr_mr_approval_edit.php?pg_id=<?php echo $result_datos_mr_approval['Id_ncr_mr_approval']; ?>&return_id=<?php echo $_REQUEST['pg_id']; ?>"><i class="bi bi-pencil" style="padding-right: 4px;"></i></a>
															<a href="/includes/ncr_mr_approval_delete.php?pg_id=<?php echo $result_datos_mr_approval['Id_ncr_mr_approval']; ?>&return_id=<?php echo $_REQUEST['pg_id']; ?>"><i class="bi bi-trash" style="padding-right: 4px;"></i></a>
														</td>
													</tr>
												<?php
												}
	...
	<script>
		function AgregarMas3() {
			$("<td>").load("includes/inputs-dinamicos-ncr_mr_approval-update.php", function() {
				$("#mr_approval").append($(this).html());
			});
		}

		function BorrarRegistro3() {
			$('tr.campos_mr_approval').each(function(index, item) {
