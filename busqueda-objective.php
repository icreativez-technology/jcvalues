...
$consulta = "SELECT objective.*, created.First_Name, created.Last_Name, approved.First_Name, approved.Last_Name FROM objective 
LEFT JOIN Basic_Employee as created ON objective.created_by = created.Id_employee
LEFT JOIN Basic_Employee as approved ON objective.approved_by = approved.Id_employee 
...
if ($consultaBD && strlen($termino) > 0) {
    ...
    while ($result_data = mysqli_fetch_assoc($consultaBD)) {
        ...
        echo "</td>";
        echo '<td><a href="/objective-view.php?id=' . $result_data['id'] . '"><i class="fa fa-eye" aria-hidden="true"></i></a>&nbsp;';
        ?>
